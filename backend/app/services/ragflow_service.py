...
def _mock_summary_answer(question_text: str) -> str:
    ...
    if is_grade:
        body = (
            ...
            "跨班共同弱點集中在「溶解現象與守恆」（INe-Ⅱ-3-02）以及「酸鹼判斷方法」（INe-Ⅲ-5-4）這兩個節點，"
            "建議優先安排補救教學。\n\n"
            "從文獻角度，這些迷思的根源往往來自學生「以視覺判斷物質存在」與「以日常感官經驗判斷酸鹼」兩種認知偏好"
            "（陳淑筠, 2003；Driver, 1985；Prieto et al., 1989）。\n\n"
            "優先介入順序：五年丙班 → 五年乙班 → 五年甲班。\n\n"
            "行動建議：\n"
            "1. 對 M03-* 持有者派發 scenario-002（飽和糖水甜度）釐清「攪拌與溶解」相關迷思\n"
            "2. 對所有班級播放「砂糖溶解後重量守恆」實驗影片，建立守恆概念\n"
        ...
        body = (
            ...
            "本班學生在 INe-Ⅱ-3-02 節點通過率僅 40%，明顯低於該節點的 70% 掌握門檻。"
            "對照文獻，這是國小學童典型的守恆認知缺陷（陳淮璋, 2002；Slone & Bokhurst, 1992）。\n\n"
            "高頻迷思優先順序：M02-1 → M09-1 → M03-1。\n\n"
            "行動建議：\n"
            "1. 對 M03-* 持有者派發 scenario-002（飽和糖水甜度）釐清「攪拌與溶解」迷思\n"
            "2. 對 M02-1 持有者單獨補強「物質守恆」實驗活動\n"
            "3. 下週複測時加入「攪拌與溶解量」的對比題以追蹤改善\n\n"
            "[REF]\n"
            "水溶液迷思概念整理_國小學童研究.docx p.41-47\n"
            "水溶液迷思概念整理_後設研究.docx 表四-3\n"
        )
