...
- **完整階層已收進版控（migration 0027，2026-06-04）**：課綱階層原本只靠 runtime docx 匯入建在 DB、不在版控，導致部署不會帶資料、各環境不一致。0027 把本地完整快照（`backend/alembic/versions/data/0027_knowledge_hierarchy.json`，units/parent_nodes/knowledge_nodes/misconceptions/unit_parent_nodes）以 **upsert** 依相依順序載入，作為最後一支 migration，讓 `alembic upgrade head` 自動帶資料（fresh 與既有 DB 皆冪等一致）。日後課綱變更需重產快照並出新版（0028…）。詳見 `docs/deviations.md`。
