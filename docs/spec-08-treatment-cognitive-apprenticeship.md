# SPEC-08: Treatment / 認知師徒制治療模組 [DEPRECATED]

> **⚠️ DEPRECATED（2026-05-25）**：經指導教授決策，整個「概念釐清・補救」模組已從實驗系統中移除。
> 前端對應頁面、路由、元件、hooks、data 已全部刪除；後端 router（`/api/scenarios/*`、`/api/treatment/*`）已自 `backend/app/main.py` 註解（實作檔保留，未來若要重新啟用可重新 mount）。
> 本文件保留作為歷史脈絡與設計參考，請勿視為當前實作依據。詳見 `docs/deviations.md`「[2026-05-25] 移除概念釐清・補救模組」。
>
> ---
>
> **角色定位（原）**：本文件是「迷思治療（概念釐清題組）」模組的單一真理來源。教師端派發、學生端對話、AI 機器人狀態機、教師端對話紀錄查閱皆以本文件為準。
>
