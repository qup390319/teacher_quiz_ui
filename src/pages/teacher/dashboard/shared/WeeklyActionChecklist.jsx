  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-1">
        ...
        <span className="text-sm text-[#95A5A6]">依緊急程度排序</span>
      </div>
            ...
            return (
              <div key={id} className={`bg-white px-5 py-4 ${idx < highFreqMiscons.length - 1 ? 'border-b border-[#D5D8DC]' : ''}`}>
                <div className="flex items-center gap-2.5 mb-2">
                  <span className="w-6 h-6 rounded-full bg-[#3D5A3E] text-white text-sm flex items-center justify-center font-bold flex-shrink-0">{idx + 1}</span>
                  <span className={`text-sm font-bold px-2.5 py-0.5 rounded-full border flex-shrink-0 ${urgency.color}`}>{urgency.label} {pct}%</span>
                  <p className="text-sm font-semibold text-[#2D3436] flex-1 min-w-0 truncate">{miscon.label}</p>
                  <span className="text-sm text-[#636E72] bg-[#EEF5E6] border border-[#D5D8DC] px-2 py-0.5 rounded-full flex-shrink-0 whitespace-nowrap">{node.name}</span>
                </div>
                ...
                {prereqNames.length > 0 && (
                  <div className="flex items-center gap-1.5 mt-1.5 ml-[34px]">
                    <span className="text-sm text-[#636E72]">先備確認：</span>
                    {prereqNames.map(name => (
                      <span key={name} className="text-sm bg-[#FCF0C2] border border-[#F5D669] text-[#B7950B] px-2 py-0.5 rounded-full">{name}</span>
                    ))}
                  </div>
                )}
              </div>
