...
- **課綱標準：** 涵蓋兩個子主題：
  - 子主題 A — 水溶液中的變化（溶解）：INe-Ⅱ-3-01 ~ INe-Ⅱ-3-05（5 個節點）
...
- **示範題組：** 四份，其中 quiz-001/002 為**單層（single）**診斷題組（各自獨立、**不是前後測**），quiz-003/004 為**雙層次（two-tier）**診斷題組；皆涵蓋同一組 5 個節點：INe-Ⅱ-3-02、INe-Ⅱ-3-03、INe-Ⅱ-3-05、INe-Ⅲ-5-4、INe-Ⅲ-5-7
- **示範班級：** 五年甲班（20 人）／五年乙班（18 人）／五年丙班（22 人）
- **診斷機制：** 兩種模式並存（詳見 spec-05 §2.2）：
  - **single 模式**：選項直接帶迷思碼；兩階段制（作答自動診斷 → 批次迷思確認）
  - **two-tier 模式**（Treagust 1988）：第一層選答案（A/B/C）+ 第二層選理由（甲/乙/丙）→ 四象限判定（TT/TF/FT/FF）；節點通過＝TT；迷思碼由理由層決定；完成後同樣接 AI 蘇格拉底追問
  - **施測中動態選題（適性診斷）**：第一層作答由後端適性引擎逐題決定下一題——過關跳過先備、答錯退回先備追溯（限題組內既有先備）；教師端報告另有跨題組的「先備概念追溯」事後定位根因。詳見 spec-10 §10.5 與 spec-02b §2.10.1。
...
- 禁止修改知識節點 ID 格式（年段一律用 **Unicode 羅馬數字** `Ⅱ`=U+2161 / `Ⅲ`=U+2162，**不得使用英文 II/III**；2026-06-03 起全庫已統一、不再並存英文寫法，詳見 `docs/deviations.md`）
